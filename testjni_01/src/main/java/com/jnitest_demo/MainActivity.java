...
public class MainActivity extends AppCompatActivity {
    ...
    @OnClick({R2.id.m_btn_crypt, R2.id.m_btn_decrypt, R2.id.m_btn_split, R2.id.m_btn_merge})
    public void onViewClicked(View view) {
        ...
        int split_count = 5;
        if (view.getId() == R2.id.m_btn_crypt) {
            try {
                CryptorUtil.crypt(normal_path, cyrpt_path);
            } catch (Throwable e) {
                e.printStackTrace();
            }
        } else if (view.getId() == R2.id.m_btn_decrypt) {
            try {
                CryptorUtil.decrypt(cyrpt_path, decyrpt_path);
            } catch (Throwable e) {
                e.printStackTrace();
            }
        } else if (view.getId() == R2.id.m_btn_split) {
            try {
                WYPFileManage.splitFile(split_path, split_count, split_text);
            } catch (Throwable e) {
                e.printStackTrace();
            }
        } else if (view.getId() == R2.id.m_btn_merge) {
